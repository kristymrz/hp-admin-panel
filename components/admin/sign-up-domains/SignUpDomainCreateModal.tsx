...
import { createSignUpDomain } from "@/app/actions/signUpDomains";

type Props = {
  onClose: () => void;
};

export default function SignUpDomainCreateModal({ onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      await createSignUpDomain(apexDomain.trim());
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Add Domain</h3>
          <button
            onClick={onClose}
            disabled={isPending}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 flex flex-col gap-5">
          <div className="flex flex-col gap-1.5">
            <label className="text-[#e8d5a3]/50 text-xs uppercase tracking-widest">
              Apex Domain <span className="text-[#ec6d70]">*</span>
            </label>
            <input
              type="text"
              value={apexDomain}
