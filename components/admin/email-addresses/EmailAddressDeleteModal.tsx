...
import { deleteEmailAddress } from "@/app/actions/emailAddresses";
...
export default function EmailAddressDeleteModal({ rowId, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    startTransition(async () => {
      await deleteEmailAddress(rowId);
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#ec6d70]/30">
          <h3 className="text-[#ec6d70] text-lg font-semibold">Delete Email Address</h3>
          <button
            onClick={onClose}
            disabled={isPending}
