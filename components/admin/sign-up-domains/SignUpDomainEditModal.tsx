...
import { updateSignUpDomain } from "@/app/actions/signUpDomains";
import type { SignUpDomainRow } from "./SignUpDomainsTable";

type Props = {
  domain: SignUpDomainRow;
  onClose: () => void;
};

export default function SignUpDomainEditModal({ domain, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      await updateSignUpDomain(domain.id, apexDomain.trim());
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Edit Domain</h3>
          <button
            onClick={onClose}
            disabled={isPending}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 flex flex-col gap-5">
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="submit"
              disabled={isPending || !apexDomain.trim()}
