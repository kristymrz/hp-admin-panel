...
import { deleteCaptionExample } from "@/app/actions/captionExamples";
...
export default function CaptionExampleDeleteModal({ exampleId, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    startTransition(async () => {
      await deleteCaptionExample(exampleId);
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#ec6d70]/30">
          <h3 className="text-[#ec6d70] text-lg font-semibold">Delete Caption Example</h3>
          <button
            onClick={onClose}
            disabled={isPending}
        ...
        <div className="px-6 py-6 flex flex-col gap-4">
          <p className="text-[#e8d5a3] text-sm leading-relaxed">
            Are you sure you want to delete this caption example? This action{" "}
