...
import { deleteSignUpDomain } from "@/app/actions/signUpDomains";
...
export default function SignUpDomainDeleteModal({ domainId, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  function handleDelete() {
    startTransition(async () => {
      await deleteSignUpDomain(domainId);
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#ec6d70]/30">
          <h3 className="text-[#ec6d70] text-lg font-semibold">Delete Domain</h3>
          <button
            onClick={onClose}
            disabled={isPending}
