...
export default function TermEditModal({ term, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const [termText, setTermText] = useState(term.term);
  const [definition, setDefinition] = useState(term.definition);
  const [priority, setPriority] = useState(String(term.priority));
  const [termTypeId, setTermTypeId] = useState(
    term.term_type_id !== null ? String(term.term_type_id) : ""
  );
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20 sticky top-0 bg-[#0f2236]">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Edit Term</h3>
          <button
            onClick={onClose}
            disabled={isPending}
