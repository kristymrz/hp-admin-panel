...
import { updateEmailAddress } from "@/app/actions/emailAddresses";
import type { EmailAddressRow } from "./EmailAddressesTable";
...
export default function EmailAddressEditModal({ row, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [emailAddress, setEmailAddress] = useState(row.email_address);

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!emailAddress.trim()) return;
    startTransition(async () => {
      await updateEmailAddress(row.id, emailAddress.trim());
      router.refresh();
      onClose();
    });
  }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Edit Email Address</h3>
          <button
            onClick={onClose}
            disabled={isPending}
