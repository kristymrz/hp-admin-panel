...
import { updateHumorFlavorMix } from "@/app/actions/humorFlavorMix";
import type { HumorFlavorMixRow } from "./HumorFlavorMixTable";

type Props = {
  row: HumorFlavorMixRow;
  onClose: () => void;
};

export default function HumorFlavorMixEditModal({ row, onClose }: Props) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const [humorFlavorId, setHumorFlavorId] = useState(String(row.humor_flavor_id));
  const [captionCount, setCaptionCount] = useState(String(row.caption_count));

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    startTransition(async () => {
      await updateHumorFlavorMix(row.id, {
        humor_flavor_id: parseInt(humorFlavorId, 10),
        caption_count: parseInt(captionCount, 10),
      });
      router.refresh();
      onClose();
    });
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Edit Humor Flavor Mix</h3>
          <button
            onClick={onClose}
            disabled={isPending}
