...
import { createLLMModel } from "@/app/actions/llmModels";

type Props = {
  onClose: () => void;
};

export default function LLMModelCreateModal({ onClose }: Props) {
  ...
  const isValid = name.trim() !== "" && providerId.trim() !== "" && providerModelId.trim() !== "";
    ...
    <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6" onClick={onClose}>
      <div className="bg-[#0f2236] rounded-lg shadow-2xl w-full max-w-xl font-[family-name:var(--font-pixelify-sans)]" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#099ff6]/20">
          <h3 className="text-[#e8d5a3] text-lg font-semibold">Add LLM Model</h3>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[#e8d5a3]/50 text-xs uppercase tracking-widest">
              Provider ID <span className="text-[#ec6d70]">*</span>
            </label>
            <input type="number" value={providerId} onChange={(e) => setProviderId(e.target.value)} disabled={isPending} className={inputClass} placeholder="e.g. 1" />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-[#e8d5a3]/50 text-xs uppercase tracking-widest">
              Provider Model ID <span className="text-[#ec6d70]">*</span>
            </label>
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="create-temp-supported" className="text-[#e8d5a3]/70 text-sm cursor-pointer">
